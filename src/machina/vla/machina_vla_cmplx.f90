...
module machina_vla_cmplx
    ...
    type, extends(machina_value) :: vla_cmplx
        private
        integer :: sz = 0
    ...
    type :: vla_cmplx_iterator
        private
        complex(kind=f8), dimension(:), pointer :: ptr => null()
        integer :: cur = 1
...
contains
    ...
    subroutine push_back_arr(this, arr)
        class(vla_cmplx), intent(inout) :: this
        complex(kind=f8), dimension(:), intent(in) :: arr
        integer :: sz, i
    ...
    pure function at(this, idx) result(v)
        class(vla_cmplx), intent(in) :: this
    ...
    pure function has_next(this) result(r)
        class(vla_cmplx_iterator), intent(in) :: this
        logical :: r
    ...
    function get_next(this) result(ptr)
        class(vla_cmplx_iterator), intent(inout), target :: this
        complex(kind=f8), pointer :: ptr
    ...
    function const_iterator(this, reverse) result(it)
        class(vla_cmplx), intent(in), target :: this
        logical, intent(in), optional :: reverse
        type(vla_cmplx_const_iterator) :: it
    ...
    pure function has_next_const(this) result(r)
        class(vla_cmplx_const_iterator), intent(in) :: this
        logical :: r
    ...
    function get_next_const(this) result(v)
        class(vla_cmplx_const_iterator), intent(inout) :: this
        complex(kind=f8) :: v
    ...
    subroutine resize(list, new_size)
        complex(kind=f8), dimension(:), allocatable, intent(inout) :: list
        integer, intent(in) :: new_size
        complex(kind=f8), dimension(:), allocatable :: tmp
        integer :: i
    ...
    function cast_to_vla_cmplx(ptr) result(v)
        class(machina_value), intent(in), target :: ptr
        type(vla_cmplx), pointer :: v
    ...
    end function cast_to_vla_cmplx

end module machina_vla_cmplx
